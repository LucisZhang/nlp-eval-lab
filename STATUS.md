...
- **Owner decision (2026-08-13, repo rule):** the ~67 MB Tier B2 int8 ONNX
  (`demo/live/tier_b2/model.int8.onnx`, 67,575,183 bytes) **stays committed in git as-is** —
  GitHub Pages cannot serve LFS objects and demo self-containment wins. It is the **only**
  large binary allowed in history; no other artifact of comparable size may be committed.
...
| Case study page (verification + "does not prove" sections) | **done** (2026-08-13, $0, derivation/presentation-only, runs.jsonl untouched at 55). Demo panel 7, data-driven via 10th contract file `demo/data/case_study.json` (DATA_CONTRACT §10): prose + declared `numbers` per section, every numeric token interpolated at build time from runs.jsonl or a committed derived artifact and test-gated — no hand-typed numbers. Narrative arc as owner-specified incl. honest negatives; §9 verification + does-not-prove sections complete; provenance + reproduce-headline render as labeled pending slots. **Owner's deferred paired CI ran first: within_A − within_B2 = +0.0168 [+0.0058, +0.0276], CI excl. 0 → sentence CERTIFIED** (robust across paths; `make prior-shift-paired`; new artifact `results/prior_shift/paired_within_…2026h1.json`; pre-existing 15 decompositions byte-identical). Tier C paired deltas materialized as committed artifacts (`results/tier_c_compare/`, `tier_c_compare --out/--pair-on shared`, 18/18 values reproduce the log). Suite **644/1/0**; `make demo-data` byte-identical ×2; browser-verified light+dark, zero console errors | EXPERIMENT_LOG 2026-08-13 (case study page) |
| Provenance links to coursework seeds | **done** (2026-08-13, $0, presentation-only, runs.jsonl untouched at 55). Case-study pending slot → real `kind: provenance` section: all 7 `docs/seed-evidence/` files carded (read-only archive — linked, never touched), 19 coursework numbers each a verbatim whole-token quotation of its archive file (build hard-fails on rounded/truncated/de-comma'd quotes; negative test proves it) under new `provenance` evidence class, framed per §9 as "self-reported class results — provenance, not evidence"; 5-lesson methodology-lineage table; Appendix-A divergences recorded in `gaps` (archive says Kaggle best **0.83522**/v1 over four submissions; 0.83615 and 0.8136 cite artifacts not in the archive → marked uncited, dev progression starts at 0.8322); group-role attribution biographical only. Push-dependent links render from single `REPO_URL_BASE` constant (empty → "link resolves after GitHub push" chips; push session fills one constant + `make demo-data`, no test edit — purity-tested). Suite **652/1/0**; `make demo-data` ×2 byte-identical (only case_study.json + meta.json changed; meta = shared legend + git_sha refresh); browser-verified light+dark, zero console errors. Only pending slot left: `reproduce_headline` | EXPERIMENT_LOG 2026-08-13 (provenance links) |
| `make reproduce-headline` | **done** (2026-08-13, $0, no new runs, runs.jsonl untouched at 55). `src/triage_lab/reproduce_headline.py`: snapshot → `make data` → **split byte-identity gate vs the frozen run records** → `predictions --force` on the chain's 11 runs → v2 cost/threshold/router/frontier → `demo_build --all` → **sha256 gate on all 27 committed outputs** (+ a file-SET check per directory; `demo/data/meta.json` compared with `git_sha` masked, mask reported). The run set is RESOLVED from the `inputs` blocks of the committed derivation files (10 runs) plus one named demo-only input (`tier_a_logreg_wordchar_cal`), cross-checked against the chain modules' own constants. Scope = headline claim chain only (drift/perturbation/OOV keep their own repro commands). Two fresh-clone blockers fixed: `make preds` missing `--extra tierb`, and the un-gitted Tier B checkpoints — preflight now hashes them against each run's `extra.checkpoint_sha256` and fails in ~2 s with the runbook path. Deviation recorded: the Tier B cost-model step passes run ids, because `--config-prefix tier_b` now also selects the Phase 5 B2 drift runs (would write 4 uncommitted files / fail on a clone). Partial run (`--skip-data --reuse-preds`) **131 s → 27/27 byte-identical**; suite **675/1/0** (+22 CI-safe tests). **Full end-to-end run executed 2026-08-13→14: `reproduce-headline: OK`, exit 0, TOTAL 46,360 s ≈ 12 h 53 m** (data 418 s, preds 45,829 s — the 3 ModernBERT seeds dominate, derivation+demo ≈114 s), snapshot sha `b4d1eac8…` verified, 11/11 artifacts re-derived + metric-verified at 1e-9, **27/27 gated outputs byte-identical to the committed records**, runs.jsonl untouched at 55 | EXPERIMENT_LOG 2026-08-13 (`make reproduce-headline`); `make reproduce-headline`, `--plan` / `--preflight` |
| Demo fully static/offline; every number traces to a results record; reproduce-headline on clean machine | **done** (2026-08-14) — demo static/offline + full traceability shipped and test-enforced; **UPGRADE_PLAN's accept line "reproduce-headline succeeds from the frozen snapshot" met by the full end-to-end run on this box** (12 h 53 m, 27/27 byte-identical, exit 0). The "clean machine" angle lands next session with the GitHub push + Actions smoke job (`--plan` is the CI-safe entry point; a CI runner has no `data/` or checkpoints by design) | EXPERIMENT_LOG 2026-08-13 (`make reproduce-headline`) |
...
4. ~~**Phase 6 — Site scaffold.** Build the static demo structure and wire in available exhibits.~~
   ...
   **done 2026-08-13** → ~~provenance links~~ **done 2026-08-13** → ~~`make reproduce-headline`~~
   **done 2026-08-13, full end-to-end run green 2026-08-14** (12 h 53 m from the frozen
   snapshot, 27/27 gated outputs byte-identical, exit 0 — Phase 6 accept line met).
...
+ disclosed in UI). ~~Case study page (verification + "does not prove")~~ — **done
2026-08-13** (EXPERIMENT_LOG 2026-08-13 case study page: panel 7 data-driven via
`case_study.json`, every number test-gated to a results record; paired within-class CI
certified at $0; Tier C paired deltas materialized as committed artifacts). Remaining
Phase 6 tasks, in order (§b item 4): ~~provenance links to coursework seeds~~ — **done
2026-08-13** (EXPERIMENT_LOG 2026-08-13 provenance links: pending slot → real section,
19 verbatim-gated archive quotations under a new `provenance` evidence class, single
`REPO_URL_BASE` constant for all push-dependent links), then ~~`make reproduce-headline`~~
— **done 2026-08-13** (EXPERIMENT_LOG 2026-08-13 `make reproduce-headline`: run set resolved
